...
class tx_mmforum_havealook {
	...
	function notifyTopicSubscribers($topicId, $forumObj) {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			...
			$forumObj->cObj->enableFields('fe_users') . ' AND tx_mmforum_topicmail.user_id != ' . intval($GLOBALS['TSFE']->fe_user->user['uid']) . $forumObj->getStoragePIDQuery('tx_mmforum_topicmail')
		);
	...
	function notifyForumSubscribers($topicId, $forumId, $forumObj) {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			...
			'tx_mmforum_forummail.user_id = fe_users.uid AND
			 (tx_mmforum_forummail.forum_id = ' . intval($forumId) . ($categoryId > 0 ? ' OR tx_mmforum_forummail.forum_id = ' . $categoryId : '') . ')' .
			 $forumObj->cObj->enableFields('fe_users') .
			 ' AND tx_mmforum_forummail.user_id != ' . intval($GLOBALS['TSFE']->fe_user->user['uid']) . $forumObj->getStoragePIDQuery('tx_mmforum_forummail')
		);
